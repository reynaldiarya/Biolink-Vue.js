<template>
  <div v-if="biolink" class="min-h-screen flex flex-col items-center justify-center font-display bg-gray-100 px-4 pt-16 pb-8">
    <img src="/assets/images/logo.png" class="w-32 h-32 rounded-full mb-6" alt="Avatar" loading="eager"
      fetchpriority="high">
  ...
  <div v-else class="min-h-screen flex flex-col items-center justify-center font-display bg-gray-100 px-4">
    <h1 class="text-3xl font-semibold mb-6">Page Not Found</h1>
    <p class="mb-6">The link you are looking for does not exist.</p>
    <router-link to="/" class="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-300">
      Return to main page
    </router-link>
  </div>
</template>
